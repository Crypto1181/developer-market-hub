...
const Payment = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch product with payment details
                const productResponse = await productsAPI.getById(id);
                const fetchedProduct = productResponse.data;
                
                // Transform Strapi field names including payment info
                const productData = {
                    ...
                    // Product-specific payment info
                    stripeLink: fetchedProduct.Stripe_Payment_Link || fetchedProduct.stripe_payment_link || '',
                    paystackLink: fetchedProduct.Paystack_Payment_Link || fetchedProduct.paystack_payment_link || '',
                    btcAddress: fetchedProduct.BTC_Wallet_Address || fetchedProduct.btc_wallet_address || '',
                    ethAddress: fetchedProduct.ETH_Wallet_Address || fetchedProduct.eth_wallet_address || '',
                    usdtAddress: fetchedProduct.USDT_Wallet_Address || fetchedProduct.usdt_wallet_address || '',
                };
    ...
    // Get wallet address based on selected network from product
    const getWalletAddress = () => {
        if (!product) return '0x742d35Cc6634C0532925a3b844Bc454e4438f44e';
        
        switch (cryptoForm.network) {
            case 'BTC':
                return product.btcAddress || '1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa';
            case 'ETH':
                return product.ethAddress || '0x742d35Cc6634C0532925a3b844Bc454e4438f44e';
            case 'USDT':
                return product.usdtAddress || product.ethAddress || '0x742d35Cc6634C0532925a3b844Bc454e4438f44e';
            default:
                return product.ethAddress || '0x742d35Cc6634C0532925a3b844Bc454e4438f44e';
        }
    };
    ...
    const handleStripeSubmit = async (e) => {
        ...
        // Redirect to product-specific Stripe payment link
        if (product.stripeLink) {
            window.location.href = product.stripeLink;
        } else {
            alert('Stripe payment link not configured for this product. Please contact support.');
        }
    };
    ...
    const handlePaystackSubmit = async (e) => {
        ...
        // Redirect to product-specific Paystack payment link
        if (product.paystackLink) {
            window.location.href = product.paystackLink;
        } else {
            alert('Paystack payment link not configured for this product. Please contact support.');
        }
    };
